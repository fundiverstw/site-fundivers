...
import { visit, chooseLanguage, watchForProblems } from './helpers'

// The page that sells FunDive to other dive shops — its own service, reached
// from the /services hub. It quotes prices, so the prices matter most here.

test.describe('the FunDive page', () => {
  test('quotes its prices', async ({ page }) => {
    const problems = watchForProblems(page)
    await visit(page, '/fundive')

    // NT$3,000 to set up, NT$3,000 a month. The NT$9,000 website price now lives
    // on its own page, so it should not appear here.
    await expect(page.getByText('NT$3,000')).toHaveCount(2)
    await expect(page.getByText('NT$9,000')).toHaveCount(0)
