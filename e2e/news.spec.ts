...
// The News feed and the article pages behind it. The unit tests
...
// The SPA fallback has to serve /news/<slug> on a cold load, and the page has
...
test('an unknown story says so instead of rendering an empty page', async ({ page }) => {
  await visit(page, '/news/no-such-story')

  await expect(page.getByText(/that story couldn.t be found/i)).toBeVisible()
})

test('the news link in the navigation opens the feed', async ({ page, isMobile }) => {
  await visit(page, '/')

  await clickNavLink(page, 'News', isMobile)
