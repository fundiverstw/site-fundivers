...
// The Logbook feed and the article pages behind it. The unit tests
...
test('the news page lists articles, newest first', async ({ page }) => {
  ...
  await visit(page, '/logbook')

  await expect(page.getByRole('heading', { name: 'The Logbook', level: 1 })).toBeVisible()
...
// The SPA fallback has to serve /logbook/<slug> on a cold load, and the page has
...
test('an unknown story says so instead of rendering an empty page', async ({ page }) => {
  await visit(page, '/logbook/no-such-story')

  await expect(page.getByText(/that story couldn.t be found/i)).toBeVisible()
})

test('the Logbook link in the navigation opens the feed', async ({ page, isMobile }) => {
  await visit(page, '/')

  await clickNavLink(page, 'The Logbook', isMobile)
