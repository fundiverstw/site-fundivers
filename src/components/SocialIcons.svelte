<script lang="ts">
  ...
  // Single-path brand glyphs (simple-icons, viewBox 0 0 24 24, fill).
  const icons: Array<{ name: string; href: string; path: string }> = [
    {
      name: 'LINE',
      href: SOCIAL.line,
      path: LINE_PATH,
...
<div class={`flex items-center gap-3 ${klass}`}>
  {#each icons as icon}
    <a
      href={icon.href}
      target="_blank"
