...
import { COURSE_DETAILS, sessionMatchesCourse } from './details'
import { COURSES } from './index'

// CourseDetail.svelte looks a write-up up with COURSE_DETAILS[course.id].
// Misspell a key and the page renders with the photo, the title, and none of the
// text you wrote — no error, anywhere. Same trap as dive-sites/details.ts.

const courseIds = COURSES.map((c) => c.id)
const entries = Object.entries(COURSE_DETAILS)
...
describe('course details', () => {
  ...
  it('cover every course', () => {
    const missing = courseIds.filter((id) => !(id in COURSE_DETAILS))
...
describe('sessionMatchesCourse', () => {
  const details = COURSE_DETAILS[Object.keys(COURSE_DETAILS)[0]]

  it('matches a category listed in the details', () => {
    for (const code of details.matchCodes) {
      expect(sessionMatchesCourse(details, code)).toBe(true)
    }
  })

  it('ignores case, because the codes are typed by hand in the booking app', () => {
    for (const code of details.matchCodes) {
      expect(sessionMatchesCourse(details, code.toUpperCase())).toBe(true)
    }
  })

  it('ignores surrounding whitespace', () => {
    expect(sessionMatchesCourse(details, `  ${details.matchCodes[0]}  `)).toBe(true)
  })

  it('does not match a category the details never claim', () => {
    expect(sessionMatchesCourse(details, 'not-a-real-code')).toBe(false)
  })

  it('returns false when there is no details.ts', () => {
    expect(sessionMatchesCourse(undefined, 'ow')).toBe(false)
  })

  it('returns false for an event with no category', () => {
    expect(sessionMatchesCourse(details, null)).toBe(false)
  })

  it('returns false for an empty category rather than matching everything', () => {
    expect(sessionMatchesCourse(details, '')).toBe(false)
    expect(sessionMatchesCourse(details, '   ')).toBe(false)
  })

  it('claims every match code for exactly one course', () => {
    // Two courses claiming the same code would list the same calendar session on
