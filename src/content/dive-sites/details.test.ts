...
import { DIVE_SITE_DETAILS } from './details'
import { DIVE_SITES } from './index'

const ids = DIVE_SITES.map((s) => s.id)
const entries = Object.entries(DIVE_SITE_DETAILS)

describe('dive-site details', () => {
  // A write-up is looked up by dive-site id. Misspell the key and the detail page
  ...
  it('cover most of the catalog', () => {
    // Not a hard requirement — a site without a details.ts degrades gracefully — but
