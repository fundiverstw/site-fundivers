...
describe('site folders', () => {
  ...
  it('are all named after a real dive site', () => {
    for (const folder of folders)
      expect(ids, `no dive site is called '${folder}'`).toContain(folder)
  })

  it('each hold a site.ts', () => {
    for (const folder of folders) {
      const files = readdirSync(resolve(import.meta.dirname, folder))
      expect(files, `${folder} has no site.ts`).toContain('site.ts')
    }
  })

  it('never hold an empty photos/ folder', () => {
    for (const folder of folders) {
      const files = readdirSync(resolve(import.meta.dirname, folder))
      if (!files.includes('photos')) continue
      const photos = readdirSync(resolve(import.meta.dirname, folder, 'photos'))
      expect(photos.length, `${folder}/photos is empty`).toBeGreaterThan(0)
    }
  })
})
