...
// The Logbook feed, served at /logbook: conferences,
...
// publishes on 14 June 2026 at /logbook/womens-dive-day. There is deliberately no
...
/** The part of an article a translator rewrites. */
 ...
 *
 *  The translations sit in this same file, under `ja` and `'zh-TW'`, and both
 *  are optional — unlike dive sites and courses, a news post ships the moment it
 *  is written in English and gets translated afterwards. An article with no
 *  entry for a language shows its English to those readers rather than blocking
 *  the post, and `npm run test:unit` prints which posts are still waiting so the
 *  backlog stays visible. A *half-filled* translation is not allowed, though:
 *  that would mix two languages inside one story. */
...
export type NewsArticle = NewsText & {
  ...
  kind: NewsKind
  photos: NewsPhoto[]
  /** Carried through from article.ts. Pages never read these directly — they go
   *  through $engine/i18n-content, which applies the English fallback. */
  ja?: NewsText
  'zh-TW'?: NewsText
}

const articleFiles = import.meta.glob('./*/article.ts', { eager: true }) as Record<
  string,
  { article?: NewsArticleFile }
>

const photoFiles = import.meta.glob('./*/*.{avif,webp,jpg,jpeg,png}', {
  eager: true,
  query: '?responsive',
  import: 'default',
}) as Record<string, ResponsiveImage>

/** The folder a globbed path sits in: './2026-06-14-x/article.ts' -> '2026-06-14-x'. */
...
/** The English text of every article, keyed by slug — what the translations are
 *  checked against. */
...
/** The articles that have been translated, keyed by slug. Deliberately sparse —
 *  see NewsArticleFile. */
const translated = (locale: 'ja' | 'zh-TW'): Record<string, NewsText> =>
  Object.fromEntries(NEWS.flatMap((a) => (a[locale] ? [[a.slug, a[locale]]] : [])))

export const newsJa: Record<string, NewsText> = translated('ja')
export const newsZhTW: Record<string, NewsText> = translated('zh-TW')
