...
<section class="mx-auto max-w-[1600px] px-4 pb-12 sm:px-6 sm:pb-16">
  <div class="grid gap-6 md:grid-cols-3">
    {#each pillars as p (p.title)}
      <div class="glass flex flex-col rounded-2xl p-7 shadow-sm">
        ...
        <h2 class="mt-4 text-t2 font-bold text-white">{p.title}</h2>
        <p class="mt-2 text-t3 leading-relaxed text-brand-100">{p.text}</p>
        <ul class="mono mt-5 space-y-2 text-t3 text-brand-50">
  ...
  <div class="glass mt-10 rounded-2xl p-8 text-center">
    <h2 class="text-t2 font-bold text-white">{$t.gear.ctaTitle}</h2>
    ...
    <p class="mono mt-5 text-t4 text-brand-200">{CONTACT.address}</p>
  </div>
</section>
