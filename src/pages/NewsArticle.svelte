<script lang="ts">
  ...
  // One article: /logbook/<slug>. The router serves this component for
  // any /logbook/<something>, so the slug is read back off the current
  // path — the same arrangement as DiveSiteDetail and CourseDetail.
  let slug = $derived($path.replace(/^\/logbook\//, '').replace(/\/+$/, ''))
